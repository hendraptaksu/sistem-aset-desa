...
1. Baca `PRD.md` (full) dan `PROGRESS.md` (full).
...
Jika `src/db/schema.*` belum ada (Agent A belum selesai), JANGAN menunggu: buat `src/features/reports/__mocks__/data.ts` dengan 20 baris dummy mengikuti tipe di atas + COA dari PRD §2, kerjakan UI + query di atas mock lewat interface `getTransaksi(filter)`. Saat schema asli ada, tinggal ganti import — logika tidak berubah.
...
- `src/features/reports/pembantu/*` — filter per kode (kas 1000-1029 atau kategori 4001-5007) + Total Masuk/Keluar/Saldo
- `src/features/reports/realisasi/*` — grouping Pendapatan per jenis (4001-4008) + Beban per Baga (5002-5007) + filter tanggal + Net = Pendapatan - Beban
- `src/features/reports/surplus/*` — SAMA dengan realisasi tapi judul `Surplus / (Defisit)` + kolom % per baris + format kurung untuk defisit
- `src/features/dashboard/*` (read saja) — Saldo Kas Tunai, Total LPD, badge Balance (baca dari query, jangan hitung ulang dengan cara beda)
...
- Semua laporan filter `tanggal` (mulai–selesai atau <= cut-off). Jangan sertakan 2050/3000 di Surplus (hanya 4xxx-5xxx).
